...
def write_pdf_data(labels, values, path):
    ...
    for i in range(labels_num):
        work_sheet_1.cell(row=1, column=i + 2).value = labels[0][i]
    for i in range(pdf_num):
        single_pdf_labels = labels[i]
        single_pdf_values = values[i]
        for j in range(labels_num):
            location = find_location(single_pdf_labels[j], labels[0])
            new_str = ''
            if len(single_pdf_values[j]) == 1:
                new_str = single_pdf_values[j][0]
                ...
                for k in range(len(single_pdf_values[j])):
                    if k == 0:
                        new_str = single_pdf_values[j][0]
                    else:
                        new_str = new_str + ';' + single_pdf_values[j][k]
                work_sheet_1.cell(row=i + 2, column=location + 2).value = new_str
    work_book.save(path)
